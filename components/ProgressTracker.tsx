...
const ProgressTracker: React.FC<ProgressTrackerProps> = ({
  ...
  return (
    <div className="h-full flex flex-col p-4 space-y-4">
      <h3 className="text-center font-bold text-slate-400 text-xs tracking-widest uppercase py-2">
        Flow Journey
      </h3>
      <div className="flex-grow min-h-0 flex flex-col space-y-2 relative overflow-y-auto pr-2 scrollbar-hide">
        {blocks.map((block, idx) => {
          ...
          const heightClass = isActive ? "h-48" : "h-24";

          return (
            <div
              key={block.id}
              className={`flex-none relative rounded-2xl border-2 transition-all duration-500 overflow-hidden flex flex-col justify-center items-center ${heightClass}
            ...
            >
              ...
              <div className="relative z-10 flex flex-col items-center text-center p-4">
                <span
                  className={`text-[10px] font-black uppercase tracking-tighter mb-1 ${block.type === "FOCUS" ? "text-blue-500" : "text-emerald-500"}`}
                >
