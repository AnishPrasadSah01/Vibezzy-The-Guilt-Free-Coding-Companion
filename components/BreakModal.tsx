...
const BreakModal: React.FC<BreakModalProps> = ({
    ...
    if (remainingSeconds < 60) {
      ...
      if (stage === "ACTIVITY") {
        setCurrentContentIndex(
          Math.floor(Math.random() * PHYSICAL_ACTIVITIES.length)
        );
      } else {
  ...
  const startStage = (newStage: BreakStage) => {
    setStage(newStage);
    const poolSize =
      newStage === "ACTIVITY" ? PHYSICAL_ACTIVITIES.length : MEMES.length;
      ...
      <div className="bg-white w-full max-w-2xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col p-10 animate-in zoom-in duration-300">
        ...
        <div className="mt-10 flex justify-between items-center">
          ...
          {stage !== "CHOOSE" && (
            ...
            >
              <span>
                {remainingSeconds < 60 ? "Done & Return" : "Give me another!"}
              </span>
              <svg
